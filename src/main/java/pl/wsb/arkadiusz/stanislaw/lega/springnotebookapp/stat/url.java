package pl.wsb.arkadiusz.stanislaw.lega.springnotebookapp.stat;

public class url {

    public static final String HOME_PAGE = "/";
    public static final String REGISTRATION_PAGE = "/registration";
    public static final String LOGIN_PAGE = "/login";
    public static final String LOGOUT_PAGE = "/logout";

    public static final String USER_PROFILE_PAGE = "/user/profile";

    public static final String JOBS_LIST_HOME_PAGE = "/jobsList/home";
    public static final String JOBS_LIST_EDIT_PAGE = "/jobsList/edit";
    public static final String JOBS_LIST_DELETE_PAGE = "/jobsList/delete";
    public static final String JOBS_LIST_NEW_PAGE = "/jobsList/new";
    public static final String JOBS_LIST_SAVE_PAGE = "/jobsList/save";
    public static final String JOBS_LIST_SAVE_UPDATE_PAGE = "/jobsList/saveUpdate";
    public static final String JOBS_LIST_CONFIRM_DELETE_PAGE = "/jobsList/confirmDelete";


}
